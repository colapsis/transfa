...
router.post('/', upload.single('file'), async (req, res) => {
  ...
  // Provenance fields
  const artifact = req.body.artifact === 'true' || req.body.artifact === true || req.body.artifact === '1' ? 1 : 0;
  const upstreamRaw = req.body.upstream_ids || req.headers['x-transfa-upstream-ids'] || null;
  let upstreamIds = null;
  if (upstreamRaw) {
    try {
      const parsed = JSON.parse(upstreamRaw);
      upstreamIds = JSON.stringify(Array.isArray(parsed) ? parsed : [parsed]);
    } catch {
      upstreamIds = JSON.stringify(upstreamRaw.split(',').map(s => s.trim()).filter(Boolean));
    }
  }
  ...
  db.prepare(
    `INSERT INTO uploads (id, api_key_id, filename, original_filename, size, sha256, mime_type, storage_path, expires_at, uploader_name, max_downloads, password_hash, run_id, step, consumer, intent, grace_seconds, source, artifact, upstream_ids)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    ...
    passwordHash,
    runId, step, consumer, intent, graceSeconds, source, artifact, upstreamIds
  );
  ...
  if (artifact)     response.artifact     = true;
  if (upstreamIds)  response.upstream_ids = JSON.parse(upstreamIds);
  if (autoGenerated) response.api_key = keyRow._generatedKey;
...
router.get('/:run_id', (req, res) => {
  ...
  const artifacts = db.prepare(
    `SELECT id, original_filename, size, sha256, mime_type, download_count,
            ...
            run_id, step, consumer, intent, artifact, upstream_ids
     FROM uploads
  ...
  res.json({
    ...
    artifacts: artifacts.map(u => ({
      ...
      artifact: u.artifact === 1,
      upstream_ids: u.upstream_ids ? JSON.parse(u.upstream_ids) : [],
      download_count: u.download_count,
...
router.delete('/:id', (req, res) => {
  ...
  if (upload.artifact && !req.query.force) {
    return res.status(409).json({ error: 'artifact is immutable — pass ?force=true to override', code: 'ARTIFACT_PROTECTED' });
  }
